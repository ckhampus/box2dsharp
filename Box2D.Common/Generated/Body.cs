...
public partial class Body : IDisposable {
  ...
  private  IntPtr  NativeCreateFixture( FixtureDef  def) {
    IntPtr cPtr = Box2DPINVOKE.Body_NativeCreateFixture__SWIG_0(swigCPtr,  ref def );
    return cPtr;
}

  private  IntPtr  NativeCreateFixture(Shape shape, float density) {
    IntPtr cPtr = Box2DPINVOKE.Body_NativeCreateFixture__SWIG_1(swigCPtr, Shape.getCPtr(shape), density);
    return cPtr;
}

  private void NativeDestroyFixture(Fixture fixture) {
    Box2DPINVOKE.Body_NativeDestroyFixture(swigCPtr, Fixture.getCPtr(fixture));
  }
